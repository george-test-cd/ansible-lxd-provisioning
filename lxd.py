...
# (c) 2017, Jiri George Holy <jiri.holy@antiluminiscent.net>
#
...
lxd_ini_path = os.environ.get('LXD_INI_PATH', os.path.join(os.path.dirname(__file__), 'lxd.ini'))
lxd_ini_path = os.path.expanduser(os.path.expandvars(lxd_ini_path))

config = configparser.SafeConfigParser()
config.read(lxd_ini_path)
...
if distutils.spawn.find_executable('lxc'):
    ...
    for item in lxdjson:
        ...
        if 'state' in item and item['state'] is not None and 'network' in item['state']:
            ...
            if 'user.ansible.groups' in item['config']:
                for user_group in item['config']['user.ansible.groups'].split(','):
                    result.setdefault(user_group, {}).setdefault('hosts',[]).append(name)
